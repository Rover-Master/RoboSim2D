...
class World:
    ...
    def checkLine(self, src: Point[float], dst: Point[float], radius=0.3) -> bool:
        """
        ...
        if False:  # Debug
            occ = self.occupancy.astype(np.uint8) * 255
            cv2.imshow(
                "Check",
                np.stack(
                    [occ, mask, ((occ > 0) & (mask > 0)).astype(np.uint8) * 255],
                    axis=-1,
                ),
            )
        return not np.any(self.occupancy & (mask > 0))
